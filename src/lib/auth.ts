...
export const forgotPassword = (data: ForgotPasswordInput): Promise<GeneralResponse> => {
    return api.post('/password/forgot-password', data);
};

export const resetPassword = (token: string, data: ResetPasswordInput): Promise<GeneralResponse> => {
    return api.post(`/password/reset-password/${token}`, data);
};

export const sendVerificationEmail = (email: string): Promise<GeneralResponse> => {
